...
namespace hotel_management_front.classes
{
    class article
    {
        ...
        public article(string reference, string designation, string familly, int quantity, int stockAlert, DateTime dateExpir, string fournisseur,
                        int prixAchat, int prixVente, string localisation, DateTime dateArrivage, double prixMoy)
        {
            this.reference = reference;
            this.designation = designation;
            this.familly = familly;
            this.prixMoyen = prixMoy;
...

        {
            ...
            if (dtbl.Rows.Count == 1)
            {
                ...
                con.Close();
                string query5 = "SELECT quantity FROM article WHERE reference=@reference";
            ...
            else
            {
                ...
                string query2 = "INSERT INTO article (reference, designation, famille, quantity, stock_alert, quantity_utilisee, date_expiration, id_fournisseur, prix_achat, prix_vente, localisation, date_arrivage , type_consommable , prix_moyen ) " +
                    "VALUES (@reference, @designation, @famille, @quantity, @stockAlert, 0, @dateExp, @idFournisseur, @prixAchat, @prixVente, @localisation, @dateArrivage , 'null' ,@prixMoy )";
                ...
                com.Parameters.AddWithValue("@prixMoy", this.prixMoyen);
        ...
        {
            string query = "SELECT reference , designation , famille , quantity, stock_alert , prix_achat ,prix_vente,localisation , type_consommable ,article.prix_moyen FROM article" +
                " INNER JOIN fournisseur ON fournisseur.id_fournisseur = article.id_fournisseur";
            con.Open();
            SqlCommand cmd = new SqlCommand(query, con);
            SqlDataAdapter adapt = new SqlDataAdapter(cmd);
        ...
        public double calculePrixReel(string Designa , int quantity , int prixAchat)
        {
            double reselt = 0;
            double reselt1 = 0;
            double reselt2 = 0;
            ...
            string query2 = "SELECT quantity,prix_moyen FROM article WHERE designation=@Desig";

            SqlDataAdapter ada1 = new SqlDataAdapter(query2, con);

            //query parameters 
            ada1.SelectCommand.Parameters.AddWithValue("@Desig", Designa);
            // command result 
            DataTable dtb = new DataTable();
            ada1.Fill(dtb);
            int quantite = int.Parse(dtb.Rows[0]["quantity"].ToString());
            quantite = quantite - quantity;
            double prix = double.Parse(dtb.Rows[0]["prix_moyen"].ToString());
            qunantiteTotal = quantite + quantity;
            reselt = prix * quantite;
            reselt1 = quantity * prixAchat;
            reselt2 = reselt + reselt1;
            reselt3 = reselt2 / qunantiteTotal;
            //return reselt;
            
            return (double)System.Math.Round(reselt3, 2);


        }
        public void modiftyPrixMoyen(string Designa, double quant)
        {
            string query = "UPDATE article SET prix_moyen=@prix WHERE designation=@Designa";
            SqlCommand com = new SqlCommand(query, con);
